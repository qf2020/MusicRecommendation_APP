...
public class NestedScrollingChild2View extends LinearLayout implements NestedScrollingChild2 {
    ...
    private final int mMinFlingVelocity;//最小滑翔速度
    private final int mMaxFlingVelocity;//最大滑翔速度
    ...
    private VelocityTracker mVelocityTracker;//跟踪触摸屏事件的速率

    @Override
    public boolean onTouchEvent(MotionEvent event) {//在NestedScrollingChild2中主要处理fling事件，因此重写onTouchEvent事件
        ...
        switch (action) {
            case MotionEvent.ACTION_UP: {//当手指抬起的时，结束嵌套滑动传递,并判断是否产生了fling效果
                ...
                int xvel = (int) mVelocityTracker.getXVelocity();//获取速率
    ...
    private boolean fling(int velocityX, int velocityY) {
        ...
        if (dispatchNestedPreFling(velocityX, velocityY)) {//判断父控件是否拦截了fling
            boolean canScroll = canScroll();
            //将fling效果传递给父控件
            dispatchNestedFling(velocityX, velocityY, canScroll); //布尔类型的canScroll表示子空间是否全部消耗了fling，如果传进去的是false，则表示没有
